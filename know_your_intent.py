...
CLASSIFIER_FILENAME_SUFFIX = '_classifier.pickle'
...
def save_models(dataset_name: str, classifiers, vectorizer) -> None:
    ...
    # Write vectorizer
    with (output_dir / VECTORIZER_FILENAME).open('wb') as pickled_file:
        pickle.dump(vectorizer, pickled_file)

    # Write classifiers
    for clf, name in classifiers:
        with (output_dir /
              (name + CLASSIFIER_FILENAME_SUFFIX)).open('wb') as pickled_file:
...
def load_models(
    ...
    for clf_path in input_dir.glob('*' + CLASSIFIER_FILENAME_SUFFIX):
        ...
        with clf_path.open('rb') as pickled_file:
            clf = pickle.load(pickled_file)
