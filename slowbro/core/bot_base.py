...
class BotBase(ABC):
    ...
    def __init__(self, round_saver_adapter: RoundSaverAdapterBase) -> None:
        """Constructor."""

        self._round_saver = RoundSaver(saver_adapter=round_saver_adapter)

    def handle_message(self, user_message: UserMessage,
        ...
        (round_index, ser_round_attributes, bot_message,
         ser_session_attributes) = self._handle_message_impl(
             user_message, ser_session_attributes)

        # stores round attributes
        self._save_round_attributes(user_message.session_id, round_index,
                                    ser_round_attributes)

        return (bot_message, ser_session_attributes)
    ...
    def handle_exception(self, user_message: UserMessage,
        ...
        slowbro_logger = SlowbroLogger(logger=logger,
    ...
    def _save_round_attributes(self, session_id: str, round_index: int,
                               ser_round_attributes: Dict[str, Any]):
        """Saves round attributes.
        """

        self._round_saver.save_round(session_id=session_id,
