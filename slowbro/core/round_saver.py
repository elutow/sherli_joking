...
from .dynamodb_utils import (dump_item_to_dynamodb, load_item_from_dynamodb)
...
class RoundSaverAdapterBase(ABC):
    ...
    @abstractmethod
    def save_round(self, session_id: str, round_index: int,
                   round_attributes: Dict[str, object]) -> None:
        pass

    @abstractmethod
    def get_round(self, session_id: str, round_index: int,
...
class RoundSaver():
    ...
    def __init__(self, saver_adapter: RoundSaverAdapterBase) -> None:
        """Constructor."""
    ...
    def save_round(self, session_id: str, round_index: int,
                   round_attributes: Dict[str, object]) -> None:
        self._saver_adapter.save_round(session_id=session_id,
    ...
    def get_round(self, session_id: str, round_index: int,
        ...
        return self._saver_adapter.get_round(session_id=session_id,
                                             round_index=round_index,
...
class DynamoDbRoundSaverAdapter(RoundSaverAdapterBase):
    ...
    def save_round(self, session_id: str, round_index: int,
        ...
        logger.debug('DynamoDbRoundSaverAdapter.save_round: %s',
                     round_attributes)
        ...
        try:
            ...
            raise RoundSaverException("DynamoDb table {} doesn't exist. "
    ...
    def get_round(self, session_id: str, round_index: int,
        ...
        if attribute_names:
            projection_expression = ','.join(
                ['attributes.{}'.format(name) for name in attribute_names])
    ...
    def _create_table_if_not_exists(self, dynamodb_resource) -> None:
        """Creates table in Dynamodb resource if it doesn't exist.
        """
        try:
            dynamodb_resource.create_table(TableName=self._table_name,
                                           KeySchema=[
                                               {
                                                   'AttributeName':
                                                   'sessionId',
