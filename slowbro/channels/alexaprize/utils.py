...
from slowbro.core.user_message import (UserMessage, AsrHypothesisToken,
...
def build_response(bot_message: BotMessage,
    ...
    if bot_message.response_ssml is not None:
        response_builder.speak(bot_message.response_ssml)

    if (bot_message.card_title is not None
            or bot_message.card_content is not None):
        response_builder.set_card(
            SimpleCard(bot_message.card_title, bot_message.card_content))

    if bot_message.reprompt_ssml is not None:
        response_builder.ask(bot_message.reprompt_ssml)

    response_builder.set_should_end_session(bot_message.should_end_session)
